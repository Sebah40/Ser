...
@Component({
        ...
        <div class="two-column-layout">
          ...
          <div class="main-column">
            <div class="text-content" [class.visible]="isVisible">
              ...
              <p class="description">
                Brindamos asesoría, consultoría e instalación de soluciones con
                el propósito de proporcionar servicios enfocados en eficiencia
              ...
              <div class="tabs-container">
                <div class="tabs-header">
                  ...
                  <button routerLink="/lista" class="view-projects-btn">
          ...
          <div class="energy-stats-column">
            <!-- Title for Energy Stats -->
            <div class="energy-stats-title">
              <h2>Energía total generada por nuestras instalaciones</h2>
              <p class="energy-stats-subtitle">
                Impacto acumulado de todos nuestros sistemas instalados
              </p>
            </div>
            <div class="energy-stats-container" *ngIf="!isLoading && data">
              <div class="energy-stat-card">
                ...
                <div class="stat-content">
                  ...
                  <div class="stat-value1">
                    {{ (data.etotalSum / 1000).toFixed(1) }} MWh
                  </div>
                </div>
              </div>
              ...
              <div class="energy-stat-card">
                <div class="stat-icon">
                <img src="/RigelecSvgs/huella de carbón blanco.svg" alt="Red Electrica Icon" />
                </div>
                <div class="stat-content">
                  <h3>Reducción de CO2</h3>
                  <div class="stat-value1">
                    {{
                      calculateCO2Reduction(data.etotalSum / 1000).toFixed(1)
                    }}
                    toneladas
                  </div>
                </div>
              ...
              <div class="energy-stat-card">
                ...
                <div class="stat-content">
                  ...
                  <div class="stat-value1">
                    {{
                      calculateTreesEquivalent(data.etotalSum / 1000).toFixed(0)
                    }}
                  </div>
                </div>
              </div>
  ...
  styles: [
    `
      ...
      .tabs-header img {
        width: 30px; /* Adjust this value as needed to scale the image */
  height: 30px; /* Keep the image square */
...
.view-projects-btn {
  ...
  cursor: pointer;
  transition: all 0.3s ease;
  box-shadow: 0 4px 15px rgba(12, 69, 122, 0.2);
}

.view-projects-btn:hover {
  transform: translateY(-3px);
  box-shadow: 0 8px 25px rgba(12, 69, 122, 0.3);
}
...
.view-projects-btn:hover i:last-child {
  transform: translateX(5px);
}
...
export class AboutComponent implements OnInit, OnDestroy {
  ...
  private readonly CO2_PER_KWH = 0.4; // CO2 reduction per KWh
  private readonly CO2_PER_TREE_KG = 18.3; // CO2 reduction per tree in kg
  ...
  private fetchData(): void {
    this.isLoading = true;
    const subscription = this.growattService.getData().subscribe({
      next: (response) => {
        if (response) {
          this.data = response;
        }
  ...
  private startPolling(): void {
    const pollingSubscription = interval(60000)
      .pipe(switchMap(() => this.growattService.getData()))
      .subscribe({
        next: (response) => {
          if (response) {
            this.data = response;
  ...
  calculateCO2Reduction(totalEnergy: number): number {
    // CO2 reduction is in kilograms, not tons, so adjust the calculation
    const co2InKg = totalEnergy * this.CO2_PER_KWH * 1000; // Convert from tons to kilograms
    return co2InKg;
  }

  calculateTreesEquivalent(totalEnergy: number): number {
    const co2Reduction = this.calculateCO2Reduction(totalEnergy);
    // Divide by 18.3 kg per tree to get the number of trees
    return co2Reduction / this.CO2_PER_TREE_KG;
  }
